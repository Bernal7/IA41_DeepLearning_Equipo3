""" Hands On - Laboratory 2 / preview file """
...
#similitud(usuario1, u2) = qué tan parecidos son dos usuarios según sus calificaciones
...
def similitud(usuario1, u2):
    suma = 0 #todas las diferencias de cudrados en la operacion
    for item in usuarios[usuario1]: #item es la representacion de cada serie
        if usuarios[usuario1][item] is not None and usuarios[u2][item] is not None: #Se asegura de comparar solo los ítems donde ambos usuarios dieron calificación.
            suma += math.pow(usuarios[usuario1][item] - usuarios[u2][item], 2) #calcula la diferencia de calificaciones al cuadrado
